...
ggsave("gpt4_human_writing_spectrum_smoothed.pdf", plot=p, width=5, height=5)
...
p <- ggplot(d.combined[type!="human"], aes(freq, power, color=type)) +
    ...
    labs(x = bquote(omega[k]), y = bquote(X(omega[k])))
ggsave("gpt4_gpt3.5_davinci_writing_spectrum_smoothed.pdf", plot=p, width=5, height=5)


# Plot gpt-4, gpt-3.5, davinci with a poor model (gpt2)
d.gpt2 <- fread("../data/demo_model.fft.txt")
...
p <- ggplot(d.combined2, aes(freq, power, color=type)) +
    ...
    labs(x = bquote(omega[k]), y = bquote(X(omega[k])))
ggsave("gpt4_gpt3.5_davinci_gpt2_writing_spectrum_smoothed.pdf", plot=p, width=5, height=5)